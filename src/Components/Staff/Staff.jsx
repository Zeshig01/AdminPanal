...
//     // Determine the next ID based on the stored staff list
...
//       setCurrentId(currentId + 1); // Increment ID for the next staff member
...
function Staff() {
  ...
  const [newStaff, setNewStaff] = useState({ name: '', position: '', fname: '', phone: '', salary: '' });
  const [modalopen, setmodalopen] = useState(false);
  const [currentId, setCurrentId] = useState(4); // Set to one more than the last dummy ID
  const [searchTerm, setSearchTerm] = useState('');
